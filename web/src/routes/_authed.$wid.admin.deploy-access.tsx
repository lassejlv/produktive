...
import {
  adminDeployAccessRequestsQuery,
  useAdminDeployAccessRequests,
  useDecideDeployAccessRequest,
} from "../lib/queries";
import { lastSeen } from "../lib/status";
import type { AdminDeployAccessRequest } from "../lib/types";

export const Route = createFileRoute("/_authed/$wid/admin/deploy-access")({
  staticData: {
    title: "Admin",
    description: "Review and approve workspace requests for the deployments private preview.",
  },
  loader: ({ context }) => context.queryClient.ensureQueryData(adminDeployAccessRequestsQuery),
  component: DeployAccessAdminPage,
});

function DeployAccessAdminPage() {
  const requests = useAdminDeployAccessRequests();
  const decide = useDecideDeployAccessRequest();
  ...
  function setStatus(req: AdminDeployAccessRequest, status: "approved" | "denied") {
    decide.mutate(
      { id: req.id, status },
      {
        onSuccess: () =>
          toast.success(
            status === "approved"
              ? `Approved deployments for ${req.workspace_name}`
              : `Denied deployments for ${req.workspace_name}`,
          ),
  ...
  if (requests.isLoading && !sorted.length) {
    return (
  ...
  if (!sorted.length) {
    return (
      <EmptyState
        ...
        description="When a workspace requests deployments access, it shows up here for review."
      />
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 gap-3 px-4 py-3 lg:grid-cols-[minmax(160px,1fr)_minmax(180px,1fr)_110px_180px] lg:items-center">
      ...
      <div className="flex justify-start gap-2 lg:justify-end">
        <Button size="sm" variant="default" disabled={pending || approved} onClick={onApprove}>
          <Check size={13} /> Approve
        </Button>
        <Button size="sm" variant="ghost" disabled={pending || denied} onClick={onDeny}>
          <X size={13} /> Deny
        </Button>
      </div>
    </div>
...
function StatusBadge({ status }: { status: AdminDeployAccessRequest["status"] }) {
