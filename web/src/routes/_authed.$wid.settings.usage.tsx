...
const CLOUD_METERS = ["deploy_memory", "deploy_cpu", "deploy_volume", "object_storage"] as const;

export const Route = createFileRoute("/_authed/$wid/settings/usage")({
  staticData: {
    title: "Settings",
    description: "Plan, usage, and spend.",
  },
...
function UsagePage() {
  ...
  const { checkout } = useSearch({ from: "/_authed/$wid/settings/usage" });
  ...
  const hasDeployUsage = CLOUD_METERS.some((key) => !!billing.balances[key]);
  const resourcePeriodBalance = CLOUD_METERS.map((key) => billing.balances[key]).find(Boolean) as
    | BillingBalanceSummary
    | null
    | undefined;
  const cloudTotal = CLOUD_METERS.reduce((sum, key) => sum + (billing.balances[key]?.cost ?? 0), 0);
  const actionPending =
    ...
    <section>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <UsageTile
          label="Events"
          usage={events}
          sub={billing.billing_enabled ? nextResetText(billing.balances.events) : "Unlimited"}
        />
      </div>
    </section>
  );
    ...
    <>
      ...
      <div className="flex flex-col gap-6">
        <div className="grid gap-3 lg:grid-cols-[1.5fr_1fr] lg:items-stretch">
          ...
          <PeriodSpendPanel
            total={cloudTotal}
            periodText={nextResetText(resourcePeriodBalance ?? billing.balances.events)}
          />
        </div>
...
/** The metered cloud spend for the current period — the page's headline number. */
function PeriodSpendPanel({ total, periodText }: { total: number; periodText: string }) {
  return (
    <div className="flex flex-col justify-between rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-elev)] px-4 py-3.5 shadow-[var(--shadow-xs)]">
      <div className="text-[11px] font-medium uppercase tracking-[0.08em] text-[var(--color-fg-dim)]">
        Usage this period
      </div>
      <div className="mt-2">
        <div className="tabular text-[28px] font-medium leading-none tracking-tight text-[var(--color-fg)]">
          {formatCost(total)}
        </div>
        <p className="mt-2 text-[12px] text-[var(--color-fg-muted)]">
          {total > 0 ? "Metered compute and storage" : "No metered usage yet"} · {periodText}
        </p>
      </div>
    </div>
  );
