...
export function AiSettings() {
  ...
  const onDelete = (server: McpServer) => {
    confirm({
      ...
      description:
        "Chats will stop seeing its tools immediately. You can reconnect later.",
...
export function McpTemplatesSettings() {
  ...
  const [selectedTemplate, setSelectedTemplate] =
    useState<McpTemplate["id"] | null>(null);
  const [accessToken, setAccessToken] = useState("");
      ...
      <SettingRow label="Templates">
        <div className="grid gap-1.5">
          {MCP_TEMPLATES.map((template) => {
            const connected = servers.some((server) =>
              sameMcpUrl(server.url, template.url),
            );
                ...
                {active && template.auth === "token" && !connected ? (
                  <div className="grid gap-2 border-t border-border-subtle p-3">
                    <input
                      value={accessToken}
                      onChange={(event) => setAccessToken(event.target.value)}
                      placeholder={`${template.name} API key`}
                      type="password"
    ...
    <div className="border-b border-border-subtle py-4">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <div className="mt-1 truncate font-mono text-[12px] text-fg-faint">
            {server.url}
          </div>
