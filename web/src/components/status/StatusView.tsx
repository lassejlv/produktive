...
export function StatusView({
  ...
  return (
    ...
    >
      ...
      {groups.length > 0 && (
        <div className="mt-2.5 text-right text-[10px] text-[var(--color-fg-dim)]">
          Uptime history shown in UTC days
        </div>
      )}
...
function UptimeBar({ history }: { history: DayBucket[] }) {
  ...
  return (
    <div className="relative" onMouseLeave={() => setHover(null)}>
      ...
      {day && (
        <>
          {/* card clamps to the bar edges; the arrow keeps tracking the tick */}
          <div
