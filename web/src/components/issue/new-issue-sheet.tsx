...
const META_CHIP =
  "inline-flex h-7 max-w-full min-w-0 shrink-0 items-center gap-1.5 rounded-md border border-border-subtle bg-surface/40 px-2 text-[11px] font-medium text-fg-muted transition-colors duration-150 hover:border-border hover:bg-surface/60 hover:text-fg";

const ROUTING_SELECT =
  "h-7 rounded-md border-border-subtle bg-surface/40 px-2 text-[11px] font-medium transition-colors duration-150 hover:border-border [&>svg]:mx-0 [&>svg]:text-fg-faint";
        ...
        >
          ...
          <SheetContent>
            <div className="px-5 pt-5 pb-1">
              <input
                ...
                className="w-full bg-transparent py-0 text-[18px] font-medium leading-snug tracking-[-0.02em] text-fg outline-none placeholder:text-fg-faint"
              />
            </div>

            {parsedIssue.title && parsedIssue.title !== title.trim() ? (
              <p className="mt-1 truncate px-5 text-[11px] text-fg-faint">
                Saves as <span className="text-fg-muted">{parsedIssue.title}</span>
              </p>
            ) : null}

            <div className="relative mt-4 px-5 pb-3 pt-3">
              <div
                aria-hidden
              ...
              <textarea
                ...
                rows={5}
                className="min-h-[120px] w-full resize-y border-0 bg-transparent text-[13px] leading-relaxed text-fg outline-none placeholder:text-fg-faint"
              />
            </div>
              ...
              <div className="relative px-5 py-4">
                ...
                <ul className="m-0 flex list-none flex-col gap-0 p-0">
                  {visibleSuggestions.map(({ issue }) => (
                    <li
                      key={issue.id}
                      className="group flex items-center gap-3 rounded-md px-1 py-2 transition-colors hover:bg-surface/40"
                    >
                      <StatusIcon status={issue.status} statuses={statuses} />
                      <p className="m-0 min-w-0 flex-1 truncate text-[13px] leading-tight text-fg">
                        {issue.title}
                      </p>
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="shrink-0 text-[11px] text-accent opacity-0 transition-opacity hover:underline focus-visible:opacity-100 group-hover:opacity-100"
                      >
                        Open
                      </a>
                      <button
                        ...
                        className="grid size-7 shrink-0 place-items-center rounded-md text-[15px] leading-none text-fg-muted opacity-0 hover:bg-surface/60 focus-visible:bg-surface/60 focus-visible:opacity-100 group-hover:opacity-100"
                      >
                        ×
                      </button>
                    </li>
            ...
            <div className="relative px-5 pb-6 pt-4">
              ...
              <div className="flex flex-wrap items-start gap-x-2 gap-y-2">
                <PillSelect
                  ...
                  options={sortedStatuses(statuses).map((s) => s.key)}
                  ...
                  {attachments.map(({ id, file }) => (
                    <li key={id} className="flex items-center gap-3 py-1">
                      ...
                      <button
                        ...
                        className="grid size-5 shrink-0 place-items-center rounded text-fg-faint transition-colors hover:bg-surface/60 hover:text-fg"
          ...
          <SheetFooter>
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              >
                {isSaving ? <Spinner size={11} /> : "Create issue"}
              </Button>
            </div>
          </SheetFooter>
        </form>
