# Agent setup for `produktive`

This is a deepsec scanning workspace. Project `produktive` was just registered
(target: `..`). Setup is incomplete — `data/produktive/INFO.md`
...
2. **Fill in `data/produktive/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id produktive
pnpm deepsec process --project-id produktive
```
