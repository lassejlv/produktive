...
Currently configured project: `produktive` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/produktive/SETUP.md` to fill in
   `data/produktive/INFO.md`.
...
`--project-id produktive` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/produktive/`.
